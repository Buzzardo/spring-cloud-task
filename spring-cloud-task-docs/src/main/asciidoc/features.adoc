...
If the task ends with a SIG-INT or a SIG-TERM, the exit code is zero unless
...
Another case to close the context is when the Task Execution completes but the application does not end.
...
This closes the application's context once the task is complete, thus allowing the application to end.
